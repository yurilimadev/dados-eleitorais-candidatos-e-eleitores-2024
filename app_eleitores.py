...
candidados_24 = pd.read_csv('candidatos_2024.csv',
                            sep=';',
                            encoding='latin-1',
...
with st.sidebar:
    ...
    mes = st.selectbox(
        'Mês', dados_agrupados['DT_GERACAO_MES'].unique(), index=None)
    df_filtrado_mes = filtrar_opcoes(dados_agrupados, filtro_mes=mes)
    uf = st.selectbox(
        'UF', df_filtrado_mes['SG_UF'].unique(), index=None)
    df_filtrado_uf = filtrar_opcoes(df_filtrado_mes, filtro_uf=uf)
    municipio = st.selectbox(
        'Munícipio', df_filtrado_uf['NM_MUNICIPIO'].unique(), index=None)
